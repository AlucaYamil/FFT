...
def acquisition_loop():
    ...
    while True:
        ...
        if raw is None:
            continue
        print(f"[UDP] seq={seq}, shape={raw.shape}")   # usa “seq” → sin warning
...
def update_dashboard(_n_intervals, _rpm, iso_group):
    ...
    fig1.update_layout(xaxis_title="Tiempo (s)", yaxis_title="Acc (m/s²)", template="plotly_white")

    fig2 = go.Figure(go.Scatter(x=list(buf_t), y=list(buf_v), mode="lines"))
    fig2.update_layout(xaxis_title="Tiempo (s)", yaxis_title="Vel (mm/s)", template="plotly_white")

    fig3 = go.Figure(go.Bar(
        x=f_fft.tolist() if f_fft else [],
        y=a_fft.tolist() if a_fft else []
    ))
    fig3.update_layout(xaxis_title="Frecuencia (Hz)", yaxis_title="Amplitud", template="plotly_white")

    fig4 = go.Figure(go.Scatter(x=list(range(len(buf_rms))), y=list(buf_rms), mode="lines"))
